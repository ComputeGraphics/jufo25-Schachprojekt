    ...
    internal class Init
    {
        ...
        public static void SettingsMenu(int start)
        {
            ...
            options = new List<Option>
            {
                ...
                new($"(PHYSICAL) TOLERANCE                             {database.physical.tolerance}", () =>  WriteTemporaryMessage("Please use Arrowkeys to Adjust Value"), () => changer(7, true, standard_tolerance.IndexOf(database.physical.tolerance), standard_tolerance, out database.physical.tolerance),() => changer(7,false, standard_tolerance.IndexOf(database.physical.tolerance), standard_tolerance, out database.physical.tolerance)),
                new($"(PHYSICAL) DO CALIBRATION                        {GetState(database.physical.calib)}", () => updateVar(8, database.physical.calib, out database.physical.calib),() => updateVar(8, database.physical.calib, out database.physical.calib),() => updateVar(8, database.physical.calib, out database.physical.calib)),
                new($"(PHYSICAL) CALIBRATION RESOLUTION                {database.physical.default_calib}", () =>  WriteTemporaryMessage("Please use Arrowkeys to Adjust Value"), () => changer(9, true, standard_resolutions.IndexOf(database.physical.default_calib), standard_resolutions, out database.physical.default_calib),() => changer(9,false, standard_resolutions.IndexOf(database.physical.default_calib), standard_resolutions, out database.physical.default_calib)),
                new($"(PHYSICAL) DEFAULT CALIB AV                      {database.physical.default_av}", () =>  enterVar(10, out database.physical.default_av), () => changer(10, true, caav.IndexOf(av), caav, out database.physical.default_av),() => changer(10,false, caav.IndexOf(av), caav, out database.physical.default_av)),
                new($"(MCU)      LAUNCH RE-SELFTEST                    {GetState(database.physical.repeat_selftest)}", () => updateVar(11, database.physical.repeat_selftest, out database.physical.repeat_selftest),() => updateVar(11, database.physical.repeat_selftest, out database.physical.repeat_selftest),() => updateVar(11, database.physical.repeat_selftest, out database.physical.repeat_selftest)),
